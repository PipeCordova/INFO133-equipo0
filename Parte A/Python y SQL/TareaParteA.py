...
cur.execute("CREATE TABLE medio_prensa (ID_MEDIO VARCHAR(50), nombre VARCHAR(50), fundacion DATE, cobertura VARCHAR(50), ID_UBI VARCHAR(50), PRIMARY KEY(id_medio), FOREIGN KEY (ID_UBI) REFERENCES ubicacion(ID_UBI) );")
...
cur.execute("CREATE TABLE noticia (url_noti VARCHAR(255), xp_fecha VARCHAR(50), xp_titulo VARCHAR(50), xp_contenido VARCHAR(50), url_sw VARCHAR(255), PRIMARY KEY(url_noti), FOREIGN KEY (url_sw) REFERENCES sitio_web(url_sw) );")
